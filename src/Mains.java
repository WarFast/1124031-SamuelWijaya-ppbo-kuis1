...
class SmartTV extends Perangkat {
    ...
    public String getDeviceDetails() {
        return "SmartTV [" + getNama() + "](ID: " + getId() + ") - Daya: " + getDaya() + ".0W | Status: " + getStatus() + " | Koneksi: WIFI | Channel: " + Channel + " | Volume: " + getVolume();
    }
}
...
class SmartSpeaker extends Perangkat {
    ...
    @Override
    public String getDeviceDetails() {
        return "SmartSpeaker [" + getNama() + "](ID: " + getId() + ") - Daya: " + getDaya() + ".0W | Status: " + getStatus() + " | Koneksi: Bluetooth | Volume: " + getVolume();
    }
}
...
class SmartDoorLocker extends Perangkat {
    ...
    @Override
    public String getDeviceDetails() {
        return "SmartDoorLocker [" + getNama() + "](ID: " + getId() + ") - Daya: " + getDaya() + ".0W | Status: " + getStatus() + " | PIN: " + PIN;
    }
}
...
class MainView {
    private PerangkatController controllers;
    private Scanner scan;

    public MainView() {
        this.controllers = new PerangkatController();
    ...
    private void menuTambahDevice() {
        ...
        System.out.println("3. Smart Door Locker");
        ...
        if (type == 1) {
            ...
            SmartDoorLocker doorLocker = new SmartDoorLocker(pin, id, nama, daya, volume, "terkunci", device.SmartDoorLocker);
            controllers.addPerangkat(doorLocker);
            System.out.println("Smart Door Locker berhasil ditambahkan!");
    ...
    private void menuTampilkanDevice() {
        List<Perangkat> devices = controllers.getAllPerangkat();
        if (devices.isEmpty()) {
        ...
        System.out.println("\n--- DAFTAR DEVICE ---");
        for (Perangkat deviceItem : devices) {
            System.out.println(deviceItem.getDeviceDetails());
        }
    }
}
